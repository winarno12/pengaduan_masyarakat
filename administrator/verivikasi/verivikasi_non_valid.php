...
$result = mysqli_query($conn, "SELECT p.id_pengaduan as id_pengaduan ,m.nama as nama, p.tgl_pengaduan as tgl_pengaduan,p.foto as foto,p.isi_laporan as isi_laporan,p.status as status FROM pengaduan p JOIN masyarakat m WHERE p.nik=m.nik AND p.status is NULL");
mysqli_fetch_assoc($result);

if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $update = mysqli_query($conn, "UPDATE pengaduan SET status='0' where id_pengaduan=$id");
    if ($update) {
        header('Location:verivikasi_non_valid.php');
    }
    echo '
    <script>
...
<body>
    ...
    <div class="container">
        <center>
            <h2 class="text-danger">List Pengaduan Non Valid</h2>
        </center>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><img src="../../assets/img/<?= $res['foto']; ?>" width="100px" class="img img-thumbnail" alt=""></td>
